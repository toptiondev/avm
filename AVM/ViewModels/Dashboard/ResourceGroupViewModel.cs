...
{
    public partial class ResourceGroupViewModel : ObservableObject, IDisposable, IQueryAttributable
    {
        ...
        [ObservableProperty]
        string _pageTitle = "Azure Resources";

        readonly VmManager _vmManager;
        List<IDisposable> _subs = new List<IDisposable>();

        public ResourceGroupViewModel(VmManager vmManager)
        {
        ...
        {
            if (e.Current.Location.OriginalString.EndsWith("/resources"))
            {
            ...
            {
                Debug.WriteLine(e.Current.Location.OriginalString);
        ...
        public async Task LoadData()
        {
            ...
            {
                var result = await _vmManager.GetResourceGroupVirtualMachine(SelectedRegion);
                VirtualMachines.Clear();
                foreach (var vm in result)
                {
                    VirtualMachines.Add(vm);
        ...
        public async Task RemoveAllVms()
        {
            ...
            if (result)
            {
                if (!string.IsNullOrEmpty(SelectedRegion))
                {
                    await _vmManager.DeleteResourceGroup(SelectedRegion);
                }
                else
                {
                    await Application.Current.MainPage.DisplayAlert("Error", "Please select a region to delete", "Ok");
                }
            }
        }
        ...
        public void ApplyQueryAttributes(IDictionary<string, object> query)
        {
            ...
            {
                SelectedRegion = query["group"].ToString();
                _ = LoadData();
            }
        }
    }
